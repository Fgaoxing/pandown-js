function pandown() {
    ...
    for (var i = 0; i < pandowns.length; i++) {
        ...
        if (type == 'baidu') {
            ...
            pandowns[i].innerHTML = '<div><p class="pan-fname" style="color: #000000">' + pandowns[i].getAttribute('fname') + '</p><br><stong class="pan-fintroduce">云盘链接：' + pandowns[i].getAttribute('url') + ' 提取码：' + pandowns[i].getAttribute('pwd') + '</stong><br></div><div><button class="pan-down" style="background-color: #000000" title="复制提取码并下载" onclick="copyText(' + pandowns[i].getAttribute('pwd') + ',' + pandowns[i].getAttribute('url') + ');">复制提取码并下载</button></div>';
        } else if (type == 'yun') {
